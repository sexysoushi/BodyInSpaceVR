...
public class WorldMovment : MonoBehaviour {
	...
	void Update () {
		...
		if(AstronautMotion.comp < AstronautMotion.distanceInit && AstronautMotion.comp < 0.8f)
		{
			this.transform.position = new Vector3 (this.transform.position.x + (0.1f), this.transform.position.y , this.transform.position.z);
		}
		else if(AstronautMotion.comp >  AstronautMotion.distanceInit && AstronautMotion.comp > 1.2f)
		{
			this.transform.position = new Vector3 (this.transform.position.x - (0.1f), this.transform.position.y, this.transform.position.z);
		}
